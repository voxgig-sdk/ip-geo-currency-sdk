...
type Args struct {
	Entity string         `json:"entity" jsonschema:"api_json | currency_conversion | currency_rate | json"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.IpGeoCurrencySDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "ip-geo-currency_list",
		Description: "List records from IpGeoCurrency. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "ip-geo-currency_load",
		Description: "Load a single record from IpGeoCurrency. " +
...
func runOp(client *sdk.IpGeoCurrencySDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.IpGeoCurrencySDK, name string) (sdk.IpGeoCurrencyEntity, error) {
	switch strings.ToLower(name) {
	case "api_json":
		return client.ApiJson(nil), nil
	case "currency_conversion":
		return client.CurrencyConversion(nil), nil
	case "currency_rate":
		return client.CurrencyRate(nil), nil
	case "json":
