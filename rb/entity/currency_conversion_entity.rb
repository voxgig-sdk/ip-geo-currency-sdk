...
class CurrencyConversionEntity
  ...
  def make
    opts = @_entopts.dup
    CurrencyConversionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpGeoCurrencyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CurrencyConversion, Hash] the current CurrencyConversion data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpGeoCurrencyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CurrencyConversion fields)
  def match_get
  ...
  #
  # @param reqmatch [CurrencyConversionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CurrencyConversion.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CurrencyConversion, Hash] the loaded CurrencyConversion; raises IpGeoCurrencyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpGeoCurrencyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
