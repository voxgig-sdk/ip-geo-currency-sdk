...
//    only - a typed IpGeoCurrencyContext there would make every ctx
...
final class IpGeoCurrencyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpGeoCurrencyContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new IpGeoCurrencySpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new IpGeoCurrencyResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new IpGeoCurrencyError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new IpGeoCurrencyResponse($clean['response']);
    ...
    public static function err_from_map($m): ?IpGeoCurrencyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpGeoCurrencyError(is_string($code) ? $code : '', $msg);
    }
