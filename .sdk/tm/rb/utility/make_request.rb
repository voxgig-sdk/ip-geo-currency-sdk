...
module IpGeoCurrencyUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpGeoCurrencyResponse.new({})
    ...
    if fetch_err
      ...
      response = IpGeoCurrencyResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpGeoCurrencyResponse.new(fetched)
    else
