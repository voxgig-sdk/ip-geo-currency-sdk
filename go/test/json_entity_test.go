...
func TestJsonEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IP_GEO_CURRENCY_TEST_JSON_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		jsonRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.json")))
...
func jsonBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IP_GEO_CURRENCY_TEST_JSON_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IP_GEO_CURRENCY_TEST_JSON_ENTID"])
