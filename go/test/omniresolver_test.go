...
import (
	...
	omni "github.com/voxgig-sdk/ip-geo-currency-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpGeoCurrencyError); is {
				out := map[string]any{
					"name":    "IpGeoCurrencyError",
...
func omniCtx(arg any, client *sdk.IpGeoCurrencySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpGeoCurrencySDK); is {
			client = live
