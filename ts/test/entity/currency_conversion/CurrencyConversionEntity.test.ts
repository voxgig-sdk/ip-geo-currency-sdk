...
describe('CurrencyConversionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'currency_conversion.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['currency_conversion01','currency_conversion02','currency_conversion03','api_rate01','api_rate02','api_rate03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
