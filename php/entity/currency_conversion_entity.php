...
// IpGeoCurrency SDK CurrencyConversion entity
...
class CurrencyConversionEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "currency_conversion";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new CurrencyConversionEntity($this->_client, $opts);
    }

    /**
     * @param CurrencyConversion|array $args CurrencyConversion data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpGeoCurrencyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CurrencyConversion|array The current CurrencyConversion data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CurrencyConversion fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpGeoCurrencyHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CurrencyConversion fields).
     */
    ...
    /**
     * Load a single CurrencyConversion.
     *
     * @param CurrencyConversionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CurrencyConversionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CurrencyConversion|array The loaded CurrencyConversion as an assoc-array at the
     *   SDK boundary; throws IpGeoCurrencyError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpGeoCurrencyHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
