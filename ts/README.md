...
The TypeScript SDK for the IpGeoCurrency API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.ApiJson()` — each with a small set of operations (`load`)
...
```ts
const currency_conversion = await client.CurrencyConversion().load({ amount: 1, base: 'base', target: 'target' })
```
...
```ts
const currency_rate = await client.CurrencyRate().load()
```
