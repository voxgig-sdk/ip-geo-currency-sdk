...
// IpGeoCurrency SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class IpGeoCurrencyTestFeature extends IpGeoCurrencyBaseFeature
{
    ...
    public function init(IpGeoCurrencyContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (IpGeoCurrencyContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = IpGeoCurrencyParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (IpGeoCurrencyContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(IpGeoCurrencyContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = IpGeoCurrencyParam::call($ctx, $k);
