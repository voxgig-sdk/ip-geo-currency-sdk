// Command IpGeoCurrency-cli is a CLI + REPL for the IpGeoCurrency SDK,
...
const prompt = "ip-geo-currency"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "api_json currency_conversion currency_rate json"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IP_GEO_CURRENCY_APIKEY (api key), IP_GEO_CURRENCY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IP_GEO_CURRENCY_APIKEY carries the API key and
	// IP_GEO_CURRENCY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_GEO_CURRENCY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_GEO_CURRENCY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpGeoCurrencySDK(opts)
