...
module IpGeoCurrencyOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if IpGeoCurrencyOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = IpGeoCurrencyOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| IpGeoCurrencyOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || IpGeoCurrencyOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = IpGeoCurrencySpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = IpGeoCurrencyError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { IpGeoCurrencyOmni.errify(err) }
