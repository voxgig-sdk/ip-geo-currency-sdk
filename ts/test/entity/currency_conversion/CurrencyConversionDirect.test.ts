...
describe('CurrencyConversionDirect', async () => {
  ...
  test('direct-load-currency_conversion', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-currency_conversion', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IP_GEO_CURRENCY_TEST_CURRENCY_CONVERSION_ENTID']
