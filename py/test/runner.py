...
class IpGeoCurrencyTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpGeoCurrencyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpGeoCurrencyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpGeoCurrencyTestRunner.getenv("IP_GEO_CURRENCY_TEST_LIVE")
        override = IpGeoCurrencyTestRunner.getenv("IP_GEO_CURRENCY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpGeoCurrencyTestRunner.getenv(key)
        ...
        explain = IpGeoCurrencyTestRunner.getenv("IP_GEO_CURRENCY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if IpGeoCurrencyTestRunner._test_control is not None:
            return IpGeoCurrencyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpGeoCurrencyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpGeoCurrencyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpGeoCurrencyTestRunner.load_test_control()
...
def load_env_local():
    IpGeoCurrencyTestRunner.load_env_local()


def env_override(m):
    return IpGeoCurrencyTestRunner.env_override(m)


def entity_data(v):
    return IpGeoCurrencyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpGeoCurrencyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpGeoCurrencyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpGeoCurrencyTestRunner.load_test_control()


def live_client_options():
    return IpGeoCurrencyTestRunner.live_client_options()


def live_delay_ms():
    return IpGeoCurrencyTestRunner.live_delay_ms()
