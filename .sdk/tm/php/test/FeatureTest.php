...
// IpGeoCurrency SDK feature test
//
...
class FtCtrl extends IpGeoCurrencyControl
{
...
class FtClient
{
    ...
    private IpGeoCurrencyUtility $utility;

    public function __construct(IpGeoCurrencyUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpGeoCurrencyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpGeoCurrencyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpGeoCurrencyContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpGeoCurrencyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpGeoCurrencyConfig::make_feature($name);
    ...
    private static function buildUrl(IpGeoCurrencySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpGeoCurrencyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpGeoCurrencyResponse($fetched);
        }
    ...
    private function fail(IpGeoCurrencyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpGeoCurrencyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
