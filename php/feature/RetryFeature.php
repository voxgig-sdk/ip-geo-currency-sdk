...
// IpGeoCurrency SDK retry feature
...
class IpGeoCurrencyRetryFeature extends IpGeoCurrencyBaseFeature
{
    ...
    public function init(IpGeoCurrencyContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpGeoCurrencyContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpGeoCurrencyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpGeoCurrencyContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
