...
// IpGeoCurrency SDK pipeline test
//
...
{
    private static function utility(): IpGeoCurrencyUtility
    {
        return new IpGeoCurrencyUtility();
    }
    ...
    private static function ctx(array $over = []): IpGeoCurrencyContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IpGeoCurrencyContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IpGeoCurrencyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IpGeoCurrencyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IpGeoCurrencyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IpGeoCurrencyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IpGeoCurrencyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IpGeoCurrencyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IpGeoCurrencyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IpGeoCurrencyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpGeoCurrencySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IpGeoCurrencyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IpGeoCurrencyResponse([]),
            'result' => new IpGeoCurrencyResult([]),
        ]);
        [, $err] = IpGeoCurrencyMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new IpGeoCurrencySpec([]),
            'response' => null,
            'result' => new IpGeoCurrencyResult([]),
        ]);
        [, $err] = IpGeoCurrencyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IpGeoCurrencySpec(['step' => 's']),
            'response' => new IpGeoCurrencyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IpGeoCurrencyResult([]),
        ]);
        [, $err] = IpGeoCurrencyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IpGeoCurrencySpec(['step' => 's']),
            'response' => new IpGeoCurrencyResponse(self::resp(200, ['v' => 1])),
            'result' => new IpGeoCurrencyResult([]),
        ]);
        [, $err] = IpGeoCurrencyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IpGeoCurrencySpec(['step' => 's']),
            'response' => new IpGeoCurrencyResponse(self::resp(200, ['v' => 2])),
            'result' => new IpGeoCurrencyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IpGeoCurrencyMakeResponse::call($ctx);
    ...
    {
        $preset = new IpGeoCurrencyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IpGeoCurrencySpec([]),
            'response' => new IpGeoCurrencyResponse([]),
            'result' => new IpGeoCurrencyResult([]),
        ...
        [$got, $err] = IpGeoCurrencyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IpGeoCurrencyResult([])]);
        [, $err] = IpGeoCurrencyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IpGeoCurrencySpec([]), 'result' => null]);
        [, $err] = IpGeoCurrencyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpGeoCurrencySpec(['step' => 's']),
            'result' => new IpGeoCurrencyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IpGeoCurrencyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpGeoCurrencyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpGeoCurrencySpec(['step' => 's']),
            'result' => new IpGeoCurrencyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IpGeoCurrencyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpGeoCurrencyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpGeoCurrencyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IpGeoCurrencySpec([]),
            'result' => new IpGeoCurrencyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IpGeoCurrencyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IpGeoCurrencyMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IpGeoCurrencyError('boom', 'boom');
        $utility->fetcher = function (IpGeoCurrencyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpGeoCurrencySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IpGeoCurrencyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IpGeoCurrencyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IpGeoCurrencyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IpGeoCurrencyResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        IpGeoCurrencyMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IpGeoCurrencyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpGeoCurrencySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IpGeoCurrencyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IpGeoCurrencyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IpGeoCurrencySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IpGeoCurrencyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IpGeoCurrencyMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IpGeoCurrencyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IpGeoCurrencyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IpGeoCurrencyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IpGeoCurrencyResult(['ok' => false])]);
        $this->expectException(IpGeoCurrencyError::class);
        IpGeoCurrencyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IpGeoCurrencyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IpGeoCurrencyMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        IpGeoCurrencyMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IpGeoCurrencyBaseFeature();
        $b = new IpGeoCurrencyBaseFeature();
        IpGeoCurrencyFeatureAdd::call($ctx, $a);
        IpGeoCurrencyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IpGeoCurrencyBaseFeature
    {
        $f = new IpGeoCurrencyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IpGeoCurrencyFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IpGeoCurrencyContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IpGeoCurrencySpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IpGeoCurrencyPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        IpGeoCurrencyPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IpGeoCurrencyResponse(['headers' => null]),
            'result' => new IpGeoCurrencyResult([]),
        ]);
        IpGeoCurrencyResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        IpGeoCurrencyResultBody::call($ctx);
