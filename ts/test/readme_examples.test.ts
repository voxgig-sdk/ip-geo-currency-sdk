...
//     (\`new IpGeoCurrencySDK(...)\` / \`IpGeoCurrencySDK.test(...)\` become
//     \`IpGeoCurrencySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpGeoCurrencySDK'
...
const TEST_SEED = {"entity":{"api_json":{"test01":{"id":"test01"}},"currency_conversion":{"test01":{"id":"test01"}},"currency_rate":{"test01":{"id":"test01"}},"json":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpGeoCurrencySDK(...)\` and any
// \`IpGeoCurrencySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpGeoCurrencySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpGeoCurrencySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
