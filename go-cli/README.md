...
boru-driven command-line client **and** interactive REPL for the IpGeoCurrency
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geo-currency-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-geo-currency-cli --help

# 3. Provide credentials once, via the environment
export IP_GEO_CURRENCY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-geo-currency-cli load 1 api_json            # {id:1} shorthand
./ip-geo-currency-cli load '{id:1}' api_json       # explicit match map

# 5. Override the API base URL for a single call
IP_GEO_CURRENCY_BASE=https://api.example.com ./ip-geo-currency-cli load 1 api_json

# 6. No arguments -> interactive REPL
./ip-geo-currency-cli
ip-geo-currency> load 1 api_json
ip-geo-currency> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geo-currency-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-geo-currency-cli load 1 api_json
   ```
...
```sh
./ip-geo-currency-cli load 1 api_json          # scalar shorthand for {id:1}
./ip-geo-currency-cli load '{id:1}' api_json     # explicit match map
```
...
./ip-geo-currency-cli load 1 api_json
```
...
Run with no arguments to open a REPL (prompt `ip-geo-currency>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-geo-currency-cli
ip-geo-currency> load 1 api_json
ip-geo-currency> /help
ip-geo-currency> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `api_json`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-geo-currency-cli`. |
...
(`./ip-geo-currency-cli <expr>`) and interactively (the REPL), and expressions compose the
