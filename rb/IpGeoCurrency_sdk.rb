...
require_relative 'IpGeoCurrency_types'
...
class IpGeoCurrencySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IpGeoCurrencyHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IpGeoCurrencyHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IpGeoCurrencyFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IpGeoCurrencyUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IpGeoCurrencyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IpGeoCurrencyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IpGeoCurrencyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = IpGeoCurrencyHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IpGeoCurrencyError.new(
        "graphql_error", "IpGeoCurrencySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IpGeoCurrencySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
