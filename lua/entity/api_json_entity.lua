...
local ApiJsonEntity = {}
ApiJsonEntity.__index = ApiJsonEntity


function ApiJsonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ApiJsonEntity)
  self._name = "api_json"
  self._client = client
...
function ApiJsonEntity:get_name()
  return self._name
end


function ApiJsonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ApiJsonEntity.new(self._client, opts)
end
...
function ApiJsonEntity:mark_deleted()
  self._deleted = true
end


function ApiJsonEntity:deleted()
  return true == self._deleted
end


function ApiJsonEntity:data_set(args)
  if args ~= nil then
...
function ApiJsonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ApiJsonEntity:match_set(args)
  if args ~= nil then
...
function ApiJsonEntity:match_get()
...
function ApiJsonEntity:stream(action, args, callopts)
...
---@param reqmatch ApiJsonLoadMatch
---@param ctrl? table
---@return ApiJson
---@return string? err
function ApiJsonEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ApiJsonEntity:_run_op(ctx, post_done)
  local utility = self._utility
