...
The Python SDK for the IpGeoCurrency API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.ApiJson(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.IpGeoCurrency(None).load(
    {"id": "test01"}, None
)
