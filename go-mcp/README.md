...
[MCP](https://modelcontextprotocol.io) server exposing the IpGeoCurrency SDK as
two agent tools — `ip-geo-currency_list` and `ip-geo-currency_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geo-currency-mcp)
make build

# 2. Provide credentials via the environment
export IP_GEO_CURRENCY_APIKEY=sk_live_xxx
...
```jsonc
// ip-geo-currency_list: first page of records
{ "entity": "api_json" }
{ "entity": "api_json", "query": { } }

// ip-geo-currency_load: one record by id
{ "entity": "api_json", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geo-currency-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-geo-currency \
     -- "$PWD"/dist/*/ip-geo-currency-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-geo-currency_list` and `ip-geo-currency_load` tools now appear
   in new sessions. Ask the agent to *"list api_json using ip-geo-currency"*
   and it calls `ip-geo-currency_list` with `{"entity":"api_json"}`.
...
```sh
export IP_GEO_CURRENCY_APIKEY=sk_live_xxx            # API key
export IP_GEO_CURRENCY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-geo-currency_list` tool
...
### Call the `ip-geo-currency_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-geo-currency_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-geo-currency_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_GEO_CURRENCY_APIKEY` | API key sent with every request. |
| `IP_GEO_CURRENCY_BASE` | Optional override of the API base URL. |
