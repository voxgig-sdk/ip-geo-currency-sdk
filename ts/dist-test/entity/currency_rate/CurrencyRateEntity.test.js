...
(0, node_test_1.describe)('CurrencyRateEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'currency_rate.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set IP_GEO_CURRENCY_TEST_CURRENCY_RATE_ENTID JSON to run live');
            return;
        }
        ...
        let currency_rate_ref01_data = Object.values(setup.data.existing.currency_rate)[0];
        // LOAD
        const currency_rate_ref01_ent = client.CurrencyRate();
        const currency_rate_ref01_match_dt0 = {};
        const currency_rate_ref01_data_dt0 = (await currency_rate_ref01_ent.load(currency_rate_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != currency_rate_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/currency_rate/CurrencyRateTestData.json');
    ...
    let idmap = transform(['currency_rate01', 'currency_rate02', 'currency_rate03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['IP_GEO_CURRENCY_TEST_CURRENCY_RATE_ENTID'];
...
//# sourceMappingURL=CurrencyRateEntity.test.js.map